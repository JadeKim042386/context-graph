...
def test_the_plugin_manifest_has_a_name_and_a_version():
    plugin = _read_json("knowledge-map", ".claude-plugin", "plugin.json")
    assert plugin["name"] == "knowledge-map"
    assert plugin["version"]


def test_all_four_refresh_points_are_hooked():
    hooks = _read_json("knowledge-map", "hooks", "hooks.json")["hooks"]
...
def test_no_machine_specific_path_is_baked_into_the_hooks():
    ...
    hooks_text = _read_text("knowledge-map", "hooks", "hooks.json")
...
def test_the_readme_states_the_three_limits():
    readme = _read_text("knowledge-map", "README.md")
...
def test_the_skill_states_the_same_rules():
    skill = _read_text("knowledge-map", "skills", "knowledge-map", "SKILL.md")
