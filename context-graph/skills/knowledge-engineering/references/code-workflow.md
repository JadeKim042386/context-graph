...
The current final design is Git plus reviewed JSON/JSONL as the canonical layer, with a commit-bound code projection. Python uses the AST adapter and emits bounded `contains`/`imports` relations; recognized non-Python languages use a deterministic lexical adapter that emits only low-confidence candidates and no structural relations. Every node records its language, parser, confidence, duplicate-safe symbols and identity, and replayable locator. Lexical fallback is a discovery aid, not AST-level evidence: it must not imply type resolution, control-flow certainty, or runtime binding. Code graphs, Tree-sitter, SCIP, SQLite, RDF/PROV-O, HTML dossiers, and Context Packs remain optional reproducible projections. Record input commit/tree, canonical-record snapshot hash, generator version, configuration digest, and output hash. Do not make an optional projection mandatory without a paired held-out evaluation showing a performance gain.
